...
- **tmux window** — the slug is used as the window name; the agent you picked in the modal (`claude` / `codex`) launches inside it with the selected permission `MODE`.

Defaults for the agent and branch prefix live in tmux options — see [tmux options](/tmux-agent-sidebar/reference/tmux-options/) for the full list.
...
Worktree lifecycle tracking is Claude-only today — it needs the `WorktreeCreate` / `WorktreeRemove` hooks, which Codex does not emit.
